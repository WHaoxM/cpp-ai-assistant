<template>
  <div class="page-container">
    <div class="filter-card">
      <el-row :gutter="15" class="responsive-row">
        <el-col :xs="24" :sm="8" class="col-item">
          <el-input v-model="searchKeyword" placeholder="搜索题目..." prefix-icon="Search" clearable />
        </el-col>
        
        <el-col :xs="24" :sm="6" class="col-item">
          <el-select v-model="selectedType" placeholder="选择题目类型" clearable style="width: 100%">
    ...
    <div class="table-container">
      <el-table :data="filteredQuestions" style="width: 100%" row-key="id">
        <el-table-column type="expand" class="mobile-only-expand">
          <template #default="props">
            <div class="detail-box">
              <p><b>知识点:</b> {{ props.row.knowledge_point }}</p>
        ...
        <el-table-column prop="question" label="题目名称" min-width="150" show-overflow-tooltip />
        ...
        <el-table-column label="操作" width="150" fixed="right">
          <template #default="scope">
            ...
            <el-button 
              v-if="scope.row.type === 'write_code'" 
              link 
              type="success" 
              @click="viewMarkdown(scope.row)">查看MD</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  setup() {
    ...
    const handleClear = async () => {
      try {
        await ElMessageBox.confirm('确定要清空所有题目数据吗？此操作不可恢复。', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        })
        const { clearQuestions } = await import('../services/questionService')
        await clearQuestions()
        questions.value = []
        ElMessage.success('已清空所有题目数据')
        addLog('已清空所有题目数据', 'success')
      } catch (error) {
        if (error !== 'cancel') {
    ...
    // 查看Markdown内容
const viewMarkdown = (row) => {
  let markdownContent = '';
  
  if (row.type === 'write_code') {
    // 为编程题型生成更完整的Markdown内容
    markdownContent = `# ${row.question || '编程题目'}
...
`;
    ...
    // 为其他题型生成通用内容
    markdownContent = `# ${row.question || '题目'}\n\n`;
  ...
  if (markdownContent.length > 2000) {
    ...
    // 使用存储ID打开新窗口
    window.open(`/markdown-viewer?storageId=${storageId}`, '_blank');
  } else {
    // 对于较短内容，仍然使用URL参数
