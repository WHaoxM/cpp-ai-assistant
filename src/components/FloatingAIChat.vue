...
.floating-chat-container.expanded {
  ...
  height: auto;
  max-height: 70vh;
  border-radius: 12px; /* 展开后圆角 */
}

.chat-header {
  padding: 10px 16px; /* 调整内边距，确保文字居中 */
  background-color: rgba(24, 144, 255, 0.95); /* 半透明背景，提高不透明度确保文字清晰 */
  color: #fff;
  border-radius: 25px 25px 0 0; /* 非展开状态下保持圆角 */
  cursor: pointer;
  display: flex;
  justify-content: space-between;
  align-items: center;
  height: 50px; /* 固定高度，确保非展开状态下文字能显示 */
  box-sizing: border-box;
}

.floating-chat-container.expanded .chat-header {
  border-radius: 12px 12px 0 0; /* 展开后使用不同的圆角 */
}
...
.chat-title {
  ...
  font-weight: bold;
  font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; /* 优化字体 */
}
...
.system-message {
  background-color: rgba(230, 247, 255, 0.9); /* 半透明背景 */
...
.user-message {
  background-color: rgba(246, 255, 237, 0.9); /* 半透明背景 */
...
.ai-message {
  background-color: rgba(255, 247, 230, 0.9); /* 半透明背景 */
...
.chat-input-area {
  ...
  background-color: rgba(255, 255, 255, 0.7); /* 70% 透明度 */
...
.topic-panel-header {
  ...
  background-color: rgba(24, 144, 255, 0.9); /* 统一主题色调 */
  color: #fff;
...
.topic-item {
  ...
  background-color: rgba(250, 250, 250, 0.9); /* 90% 透明度，提高可读性 */
  backdrop-filter: blur(5px); /* 添加毛玻璃效果 */
}
